...
export function drawPrecedeModel() {
    ...
    const dataLine = new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(0,0,0), new THREE.Vector3(0,0,0)]), new THREE.LineBasicMaterial({ color: 0x00ffff, transparent: true, opacity: 0.5 }));
    scene.add(dataLine);
    ...
    function animate() {
        ...
        const lPos = dataLine.geometry.attributes.position.array;
        lPos[0] = player.position.x; lPos[1] = player.position.y; lPos[2] = player.position.z;
        lPos[3] = precedeObj.position.x + targetDir.x * 30; lPos[4] = precedeObj.position.y + targetDir.y * 30; lPos[5] = precedeObj.position.z;
